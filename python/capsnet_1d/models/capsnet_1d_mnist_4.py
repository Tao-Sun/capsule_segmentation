...
def inference(inputs, num_classes, routing_ites=3, remake=False, training=False, name='capsnet_1d'):
    ...
    with tf.variable_scope(name) as scope:
        ...
        primary_caps_activations, conv2 = primary_caps1d(
            conv1,
            kernel_size=7, out_capsules=primary_out_capsules, stride=1,
        ...
        conv_caps_activations = conv_capsule1d(
            ...
            kernel_size=3, stride=2, routing_ites=3,
        ...
        label_logits = _decode(
            conv_caps_activations, primary_out_capsules,
...
def _decode(activations, capsule_num, coupling_coeffs, num_classes, batch_size, conv1, conv2):
    ...
    conv_caps_labels = tf.reduce_sum(coupling_coeff_reshaped * caps_probs_tiled, 1)  # (b, 4, 20, 2)
    print('conv_caps_labels shape: %s' % conv_caps_labels.get_shape())
    ...
    concat1 = tf.concat([tf.transpose(conv2, perm=[0, 2, 3, 1]), deconv1], axis=3, name='concat1')
    primary_conv = conv2d(
        concat1,
    ...
    deconv2 = deconv(
        primary_conv,
        kernel=8, out_channels=128, stride=2,
